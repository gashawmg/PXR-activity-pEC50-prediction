"""
pEC50 PXR — Pipeline v4_3_3
============================
Base    : v4_3_2 (bug-fix branch) with Huber loss reverted

Changelog v4_3_2 -> v4_3_3
---------------------------
REVERTED Loss: Huber -> MSE (chemprop default, no custom criterion).
           Huber with delta=0.5 puts most errors (OOF MAE ~0.44) in the
           linear regime, giving a constant gradient signal.  MSE's quadratic
           penalty gives larger errors a proportionally bigger gradient, which
           helps the model converge faster on badly-wrong predictions.
           Leaderboard confirmed Huber was worse (rank 59 vs rank 50).

RESTORED Tail upweight cap: 2x -> 5x (matching v4_3_1 / v4_1 which gave
           rank 26 / rank 50).  The 2x cap in v4_3_2 was an untested change;
           reverting to the cap that produced the two best-known results.

KEPT     Sample weight bug fix from v4_3_2:
           Weights are correctly passed to MoleculeDatapoint(weight=...).
           In v4_3_1 the weights were computed but silently discarded.
...
import argparse, glob, os, pickle, random, re, time, warnings
...
TRAIN_CSV      = "clean_train1.csv"
TEST_CSV       = "test.csv"
SMILES_COL     = "SMILES"
TARGET_COL     = "pEC50"
NAME_COL       = "Molecule Name"
MODEL_SAVE_DIR = "models_pec50_pxr_v4_3_3_one"
OUTPUT_CSV     = "predictions_pec50_pxr_v4_3_3_7_one.csv"
...
def compute_sample_weights(df, min_w=0.1, max_w=5.0):
    ...
    combined = np.clip(sel_w * ci_w * tail_w, min_w, max_w)
...
class PXRMPNNPredictor:
    ...
    def predict(self, smiles_list, use_weighted=True):
        """Weighted ensemble prediction (1/MAE weights)."""
...
def main():
    ...
    parser.add_argument('--no_sample_weights', action='store_true',
                        help='Disable selectivity/CI/tail sample weighting. '
                             'Use to A/B test whether weighting helps on the '
                             'blind test set.')
    ...
    if args.mode in ('INFERENCE', 'BOTH'):
        ...
        print("Generating MPNN predictions ...")
        preds = predictor.predict(test_smiles, use_weighted=True)
        ...
        sub.to_csv(args.output, index=False)
        print(f"\nSubmission columns: {list(sub.columns)}")
        print(f"Saved → {args.output}")
        print(f"  Ready to submit: 'Molecule Name', 'SMILES', 'pEC50'")
